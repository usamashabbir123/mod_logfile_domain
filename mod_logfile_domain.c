...
static switch_bool_t is_valid_domain(const char *domain)
{
	...
	/* Reject common invalid patterns - EXPANDED LIST */
	if (strstr(domain, ".invalid") != NULL || 
		strstr(domain, "freeswitch") != NULL ||
		strstr(domain, "example.com") != NULL ||
		strstr(domain, "example.org") != NULL ||
		strstr(domain, "test.") != NULL ||
		strstr(domain, ".test") != NULL ||
		strstr(domain, "default") != NULL) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, 
				"mod_logfile_domain: Rejecting invalid/test domain: %s\n", domain);
		return SWITCH_FALSE;
	}
